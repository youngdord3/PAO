...
std::unique_ptr<FiltroStrategy> FiltroFactory::createGenereLibroFiltro(const QString& genere)
{
    auto filtroComposto = std::make_unique<FiltroComposto>();
    filtroComposto->addFiltro(std::make_unique<FiltroTipo>("Libro"));
    filtroComposto->addFiltro(std::make_unique<FiltroCriterio>("genere", genere));
    return filtroComposto;
}

std::unique_ptr<FiltroStrategy> FiltroFactory::createGenereFilmFiltro(const QString& genere)
{
    auto filtroComposto = std::make_unique<FiltroComposto>();
...
std::unique_ptr<FiltroStrategy> FiltroFactory::createCategoriaArticoloFiltro(const QString& categoria)
{
    auto filtroComposto = std::make_unique<FiltroComposto>();
    filtroComposto->addFiltro(std::make_unique<FiltroTipo>("Articolo"));
    filtroComposto->addFiltro(std::make_unique<FiltroCriterio>("categoria", categoria));
    return filtroComposto;
}

std::unique_ptr<FiltroStrategy> FiltroFactory::createLibriRecentiFiltro(int anni)
{
    int annoCorrente = QDate::currentDate().year();
    auto filtroComposto = std::make_unique<FiltroComposto>();
    filtroComposto->addFiltro(std::make_unique<FiltroTipo>("Libro"));
    filtroComposto->addFiltro(std::make_unique<FiltroAnno>(annoCorrente - anni, annoCorrente));
    return filtroComposto;
}
